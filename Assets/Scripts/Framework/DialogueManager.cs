...
public class PlayerSentence
{
    ...
    public PlayerSentence(string text, bool active, List<Sentence> conversation, List<int> enableIndices, List<int> disableIndices)
    {
        ...
        m_sentence = new Sentence(text, 0, false);
...
public class Sentence
{
    ...
    public bool m_convoEnder;
    public Sentence(string text, int portraitIndex, bool convoEnder)
    {
        m_text = "";
        m_portraitIndex = portraitIndex;
        m_convoEnder = convoEnder;
        UpdateText(text);
    }
...
public class DialogueManager : Singleton<DialogueManager>
{
    ...
    public void M_StartConversation(Dialogue dialogue)
    {
    ...
    private int M_PlaceAllActivePlayerSentences()
    {
        ...
        for (int i = m_currentDialogue.m_playerSentences.Count - 1; i >= 0; i--)
        {
            PlayerSentence ps = m_currentDialogue.m_playerSentences[i];
            if (ps.m_active)
            {
                activeCount++;
            }
    ...
    public void M_ProgressConversation()
    {
        ...
        {
            if (m_currentPlayerSentence.m_conversation[m_nextConversationSentence].m_convoEnder)
            {
        ...
        else
        {
            ...
            {
                m_currentDialogue.m_playerSentences[i].m_active = true;
            }
            // Disable options
            foreach (int i in m_currentPlayerSentence.m_disableIndices)
            {
                m_currentDialogue.m_playerSentences[i].m_active = false;
            }
            M_PlaceAllSentences();

            //if (m_currentPlayerSentence.m_sentence.m_convoEnder)
            //{
            //    m_dialoguePanel.SetActive(false);
            //}
        }
